...
## Loop Prevention — CRITICAL: CHECK FIRST

**Before any analysis**, verify the triggering issue has the `milestone-feature` label. If it does NOT have this label, respond with "No decomposition needed — missing milestone-feature label" and stop immediately.

Additionally, do nothing if:
...
```markdown
## 🔨 Feature Decomposed into Work Items

This feature has been decomposed into {count} work items:

1. #{work-item-1-number} — {brief description}
2. #{work-item-2-number} — {brief description}
3. #{work-item-3-number} — {brief description}
...
...
- **forge-milestone-lifecycle**: Manages milestone planning and daily progress tracking
...
- Maintain link integrity (parent → children, children → parent)
